...
public class ViewResultsServiceImpl implements ViewResultsService {
    ...
    @Transactional(value = "School_DBTransactionManager")
    @Override
    public ViewResults partialUpdate(ViewResultsId viewresultsId, Map<String, Object>viewResultsPatch) {
        LOGGER.debug("Partially Updating the ViewResults with id: {}", viewresultsId);

        ViewResults viewResults = getById(viewresultsId);

        try {
            ObjectReader viewResultsReader = this.objectMapper.readerForUpdating(viewResults);
            viewResults = viewResultsReader.readValue(this.objectMapper.writeValueAsString(viewResultsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", viewResultsPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        viewResults = update(viewResults);
