...
public class TestDetailsServiceImpl implements TestDetailsService {
    ...
    public TestDetails update(TestDetails testDetails) {
        ...
        List<AcademicTestSubjects> academicTestSubjectses = testDetails.getAcademicTestSubjectses();
        if(academicTestSubjectses != null && Hibernate.isInitialized(academicTestSubjectses)) {
            academicTestSubjectses.forEach(_academicTestSubjects -> _academicTestSubjects.setTestDetails(testDetails));
        }
    ...
    @Transactional(value = "School_DBTransactionManager")
    @Override
    public TestDetails partialUpdate(Integer testdetailsId, Map<String, Object>testDetailsPatch) {
        LOGGER.debug("Partially Updating the TestDetails with id: {}", testdetailsId);

        TestDetails testDetails = getById(testdetailsId);

        try {
            ObjectReader testDetailsReader = this.objectMapper.readerForUpdating(testDetails);
