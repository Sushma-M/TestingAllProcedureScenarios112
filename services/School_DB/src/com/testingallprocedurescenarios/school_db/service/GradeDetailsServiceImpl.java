...
public class GradeDetailsServiceImpl implements GradeDetailsService {
    ...
    public GradeDetails update(GradeDetails gradeDetails) {
        ...
        List<Results> resultses = gradeDetails.getResultses();
        if(resultses != null && Hibernate.isInitialized(resultses)) {
            resultses.forEach(_results -> _results.setGradeDetails(gradeDetails));
        }
    ...
    @Transactional(value = "School_DBTransactionManager")
    @Override
    public GradeDetails partialUpdate(Integer gradedetailsId, Map<String, Object>gradeDetailsPatch) {
        LOGGER.debug("Partially Updating the GradeDetails with id: {}", gradedetailsId);

        GradeDetails gradeDetails = getById(gradedetailsId);

        try {
            ObjectReader gradeDetailsReader = this.objectMapper.readerForUpdating(gradeDetails);
            gradeDetails = gradeDetailsReader.readValue(this.objectMapper.writeValueAsString(gradeDetailsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", gradeDetailsPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        gradeDetails = update(gradeDetails);
