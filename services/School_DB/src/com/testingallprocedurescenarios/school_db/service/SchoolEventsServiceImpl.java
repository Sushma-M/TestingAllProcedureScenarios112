...
public class SchoolEventsServiceImpl implements SchoolEventsService {
    ...
    @Transactional(value = "School_DBTransactionManager")
    @Override
    public SchoolEvents partialUpdate(SchoolEventsId schooleventsId, Map<String, Object>schoolEventsPatch) {
        LOGGER.debug("Partially Updating the SchoolEvents with id: {}", schooleventsId);

        SchoolEvents schoolEvents = getById(schooleventsId);

        try {
            ObjectReader schoolEventsReader = this.objectMapper.readerForUpdating(schoolEvents);
            schoolEvents = schoolEventsReader.readValue(this.objectMapper.writeValueAsString(schoolEventsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", schoolEventsPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        schoolEvents = update(schoolEvents);
    ...
    public SchoolEvents delete(SchoolEventsId schooleventsId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), SchoolEvents.class.getSimpleName(), schooleventsId);
        }
