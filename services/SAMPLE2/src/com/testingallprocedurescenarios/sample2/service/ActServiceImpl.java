...
public class ActServiceImpl implements ActService {
    ...
    @Transactional(value = "SAMPLE2TransactionManager")
    @Override
    public Act partialUpdate(Short actId, Map<String, Object>actPatch) {
        LOGGER.debug("Partially Updating the Act with id: {}", actId);

        Act act = getById(actId);

        try {
            ObjectReader actReader = this.objectMapper.readerForUpdating(act);
            act = actReader.readValue(this.objectMapper.writeValueAsString(actPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", actPatch, ex);
    ...
    public Act delete(Short actId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), Act.class.getSimpleName(), actId);
        }
