...
public class ProjactServiceImpl implements ProjactService {
    ...
    @Transactional(value = "SAMPLE2TransactionManager")
    @Override
    public Projact partialUpdate(ProjactId projactId, Map<String, Object>projactPatch) {
        LOGGER.debug("Partially Updating the Projact with id: {}", projactId);

        Projact projact = getById(projactId);

        try {
            ObjectReader projactReader = this.objectMapper.readerForUpdating(projact);
            projact = projactReader.readValue(this.objectMapper.writeValueAsString(projactPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", projactPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        projact = update(projact);
    ...
    public Projact delete(ProjactId projactId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), Projact.class.getSimpleName(), projactId);
        }
