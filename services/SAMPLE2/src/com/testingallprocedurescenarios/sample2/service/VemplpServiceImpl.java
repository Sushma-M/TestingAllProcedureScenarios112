...
public class VemplpServiceImpl implements VemplpService {
    ...
    @Transactional(value = "SAMPLE2TransactionManager")
    @Override
    public Vemplp partialUpdate(VemplpId vemplpId, Map<String, Object>vemplpPatch) {
        LOGGER.debug("Partially Updating the Vemplp with id: {}", vemplpId);

        Vemplp vemplp = getById(vemplpId);

        try {
            ObjectReader vemplpReader = this.objectMapper.readerForUpdating(vemplp);
            vemplp = vemplpReader.readValue(this.objectMapper.writeValueAsString(vemplpPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", vemplpPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        vemplp = update(vemplp);
    ...
    public Vemplp delete(VemplpId vemplpId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), Vemplp.class.getSimpleName(), vemplpId);
        }
