...
public class InTrayServiceImpl implements InTrayService {
    ...
    @Transactional(value = "SAMPLE2TransactionManager")
    @Override
    public InTray partialUpdate(InTrayId intrayId, Map<String, Object>inTrayPatch) {
        LOGGER.debug("Partially Updating the InTray with id: {}", intrayId);

        InTray inTray = getById(intrayId);

        try {
            ObjectReader inTrayReader = this.objectMapper.readerForUpdating(inTray);
            inTray = inTrayReader.readValue(this.objectMapper.writeValueAsString(inTrayPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", inTrayPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        inTray = update(inTray);
    ...
    public InTray delete(InTrayId intrayId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), InTray.class.getSimpleName(), intrayId);
        }
