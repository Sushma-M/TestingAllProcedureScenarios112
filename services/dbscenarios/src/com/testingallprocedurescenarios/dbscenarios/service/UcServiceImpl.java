...
public class UcServiceImpl implements UcService {
    ...
    @Transactional(value = "dbscenariosTransactionManager")
    @Override
    public Uc partialUpdate(Integer ucId, Map<String, Object>ucPatch) {
        LOGGER.debug("Partially Updating the Uc with id: {}", ucId);

        Uc uc = getById(ucId);

        try {
            ObjectReader ucReader = this.objectMapper.readerForUpdating(uc);
            uc = ucReader.readValue(this.objectMapper.writeValueAsString(ucPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", ucPatch, ex);
    ...
    public Uc delete(Integer ucId) {
        ...
        if (deleted == null) {
            ...
            throw new EntityNotFoundException(MessageResource.create("com.wavemaker.runtime.entity.not.found"), Uc.class.getSimpleName(), ucId);
        }
